...
// =============================================================
// 📥 PROCESS BILLS API (SUPER FAST PARALLEL AI PROCESSING)
// =============================================================
app.post('/admin/api/process-bills', checkAdmin, upload.array('billFiles', 20), async (req, res) => {
    try {
        if (!req.files || req.files.length === 0) {
            return res.json({ success: false, msg: "કૃપા કરીને ફાઇલ સિલેક્ટ કરો!" });
        }

        // 🎯 Read starting manual invoice number
        ...
        // 🚀 Fast & Stable Gemini Flash Model
        const model = genAI.getGenerativeModel({ model: "gemini-3.6-flash" });

        // ⚡ PARALLEL PROCESSING: Process each bill independently & save immediately
        const filePromises = req.files.map(async (file, idx) => {
            ...
            try {
                ...
                const prompt = `Carefully analyze this Tax Invoice document image/pdf and extract all buyer/consignee details AND all product items from the invoice table into a JSON object:
                {
                    ...
                    "raw_items": [
                        {
                            "product": "Product name e.g. SOAP or Offer Combo 2200/-",
                            "hsn": "HSN Code e.g. 30045090",
                            "qty": "Quantity e.g. 1",
                            "rate": "Rate before tax",
                            "pv": "PV points e.g. 500",
                            "offer_pv": "Offer PV if present else 0",
                            "discount": "Discount if present else 0.00",
                            "offer_discount": "Offer discount if present else 0.00",
                            "taxable_val": "Taxable Value",
                            "gst_pct": "GST percentage e.g. 18.00",
                            "gst_amt": "GST Amount",
                            "net_amt": "Net Amount"
                        }
                    ]
            ...
            // 🎯 INVOICE NO LOGIC: Incremental Numbering for Multiple Files (101, 102, 103...)
            ...
            // 🎯 Process Product Items (Combos Breakup + Single Products)
            ...
            for (let rawIt of extractedItems) {
                ...
                if (matchedCombo) {
                    let subProducts = typeof matchedCombo.products === 'string' 
            ...
            const rawComboNameSummary = extractedItems.map(i => i.product).join(', ');
            ...
            const dbRes = await db.query(insertQuery, [
                ...
                rawComboNameSummary || "",
                JSON.stringify(finalProductsList),
        ...
        // ⚡ Execute all AI scans simultaneously in parallel
...
// =============================================================
// 🖨️ BULK PRINT SELECTED BILLS API (COMBINES ALL SELECTED IN ONE PAGE)
// =============================================================
app.get('/admin/api/print-multiple-bills', checkAdmin, async (req, res) => {
    try {
        ...
        // Render multi-bill preview HTML
...
// =============================================================
// 🖨️ PERFECT 75% A4 TAX INVOICE PREVIEW ROUTE (EXACT PDF FORMAT)
// =============================================================
        ...
        <head>
            ...
            <style>
                ...
                * { box-sizing: border-box; }
                ...
                /* 📄 EXACT 75% COMPACT REAL A4 CONTAINER */
                .invoice-container { 
                    width: 210mm; 
                    min-height: 297mm; 
                    padding: 8mm; 
                    ...
                    transform: scale(0.95); /* Perfect scale fit */
